...
Depending on the nature of the delays and their influence on the system dynamics, time-delay systems can be categorized into different classes, such as retarded, neutral, and delay descriptor systems.
In ``TDSpy``, we focus on linear time-invariant (LTI) time-delay systems with multiple discrete delays, which can be represented using delay-differential equations (DDEs) or delay-differential-algebraic equations (DDAEs).
...
``TDSpy`` handles LTI retarded (autonomous) time-delay systems with multiple discrete delays, represented by delay-differential equations (DDEs) of the form:
...
.. admonition:: Example: The shower problem
    :class: example
...
The above works similar to the `TDS-Control` MATLAB function `tds_create('A',{A0,A1},'hA',[0,0.5])` for defining retarded time-delay systems.
...
A neutral time-delay system can be created using the :func:`tdspy.NDDE` class. The below example demonstrates 
how to create a neutral time-delay system in ``TDSpy``. 
...
.. admonition:: Example: See Example 1.18 from :cite:`michielsStability2007`
    ...
    To obtain the associated delay-difference equation, we can use the in-built method `get_delay_difference_equation` from the :class:`tdspy.NDDE` class as:
...
A linear time-delay descriptor system can be described by a set of delay-differential-algebraic equations (DDAEs) that take the form:
