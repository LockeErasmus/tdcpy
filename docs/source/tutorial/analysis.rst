...
`TDSpy` follows the procedure described in :cite:`WuSpectral2017` for computing the characteristic roots of retarded systems.
The procedure consists of two steps: first, a finite number of characteristic roots are computed using a spectral discretization method, and then the roots are refined using a Newton-type iteration.
The tutorial :ref:`_tutorials` provides a deeper analysis into the discretization procedure.
